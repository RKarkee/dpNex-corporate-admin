...
/**
 * The two ways a quote is shown: as one of several options to pick from, and as
 * the confirmed choice at the top of the create form.
 *
 * They share the money block, which is the part that has to read identically in
 * both — a total that looks different after selection invites a double-check
...
const transit = (days: number) => `${days} day${days === 1 ? "" : "s"}`;
  ...
  return (
    <Card className="flex flex-col gap-4 p-5 transition-shadow hover:shadow-card">
      ...
      <div className="space-y-2">
        <Row label="Transit" value={transit(rate.transit_days)} icon={Clock} />
      ...
      <div className="mb-4 flex items-start justify-between gap-2">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            The rate this consignment will be created against.
          </p>
        </div>
        <Badge variant="secondary">{rate.service_code}</Badge>
      </div>
