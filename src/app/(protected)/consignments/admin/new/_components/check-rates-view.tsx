...
import { useCheckRates } from "../../_hooks/use-consignments";
import type { RateOption } from "../../types";

/**
 * Step one of creating a consignment: price the destination, then pick a rate.
 *
 * A separate step rather than a section of the main form because the answer
 * decides the rest — the routing codes, the carrier and the price all come from
 * the quote the user selects, and none of the consignment details can be priced
 * without a destination and a weight first.
 *
 * No customer or customer-type field, unlike the reference admin portal: that
 * one serves every corporate at once, so it has to ask. Here
 * `X-Corporate-Code` already answers it.
 *
 * The selection is carried to step two through the URL. It is bulky for a query
 * string, but it survives a refresh and a back-navigation, which a module-level
 * variable or a context would not — and losing a chosen quote halfway through a
 * long form is worse than a long URL.
 */
 ...
 * result holds a number — hence two types rather than one.
 */
...
export function CheckRatesView() {
  ...
  const handleSelect = (rate: RateOption) => {
    ...
    const params = new URLSearchParams({
      ...
      receiver_country: String(values.receiver_country ?? ""),
      receiver_state: String(values.receiver_state ?? ""),
      receiver_state_name: String(values.receiver_state_name ?? ""),
      receiver_city: String(values.receiver_city ?? ""),
      receiver_zip: String(values.receiver_zip ?? ""),
      package_type: String(values.package_type ?? ""),
      package_type_label: packageTypeLabel,
    });

    router.push(`/consignments/admin/create?${params.toString()}`);
  };
        ...
        <Card className="p-6 sm:p-8">
          ...
          >
            <LocationFields
              ...
              stateValue={state}
              // A state from the old country cannot survive the change, nor can
              // a city from the old state.
              onCountryChange={() => {
        ...
        <section className="space-y-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Pick one to continue — its carrier, service and price carry through
              to the consignment.
            </p>
          </div>
