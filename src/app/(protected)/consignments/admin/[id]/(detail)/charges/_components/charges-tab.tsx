...
/**
 * The Charges tab: the cost line items on one consignment.
 *
...
export function ChargesTab({ id }: { id: number }) {
  ...
  const consignment = useConsignment(id);
  const { canAddCharges, canUpdateCharges } = useConsignmentAdminPermissions();
  const query = useConsignmentCharges(consignmentId, page, applied);
  const deleteCharge = useDeleteConsignmentCharge(consignmentId);
  const { quantityCodeOptions } = useMetaOptions();

  const record = consignment.data?.consignment;
      ...
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Every line making up this consignment&apos;s cost, system-generated and manual alike
          </p>
        </div>
        {addButton}
      </div>
