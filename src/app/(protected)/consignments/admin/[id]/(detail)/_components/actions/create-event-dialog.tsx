...
import { useCreateConsignmentEvent } from "../../../../_hooks/use-consignment-actions";

/**
 * Create Event — `POST …/events` with `{ event_code }`.
 *
 * The codes come from `/meta`'s `consignment_event_codes`: the user
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Log an event against this consignment.</DialogDescription>
        </DialogHeader>

        {open ? (
          <CreateEventForm consignmentId={consignmentId} onDone={() => onOpenChange(false)} />
        ) : null}
      </DialogContent>
    </Dialog>
  );
...
function CreateEventForm({
  ...
  const { consignmentEventCodeOptions, isPending: loadingMeta } = useMetaOptions();
  const createEvent = useCreateConsignmentEvent(consignmentId);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <FieldShell label="Event" required error={error}>
        {() => (
          <Combobox
            options={consignmentEventCodeOptions}
            value={eventCode}
            onChange={(value) => {
              setEventCode(value);
              setError(undefined);
