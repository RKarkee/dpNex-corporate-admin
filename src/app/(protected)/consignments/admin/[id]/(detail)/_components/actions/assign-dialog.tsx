...
import { useAssignConsignment } from "../../../../_hooks/use-consignment-actions";

/**
 * Assign — `POST …/assign` with `{ task_code, assigned_to }`, offered while
 * nobody holds the consignment (`current_assignee_id` is null).
 *
 * Tasks come from `/meta`'s `consignment_task_codes`; people from the
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Hand a task on this consignment to a user.</DialogDescription>
        </DialogHeader>

        {open ? (
          <AssignForm consignmentId={consignmentId} onDone={() => onOpenChange(false)} />
        ) : null}
      </DialogContent>
    </Dialog>
  );
...
function AssignForm({ consignmentId, onDone }: { consignmentId: number; onDone: () => void }) {
  const { consignmentTaskCodeOptions, isPending: loadingMeta } = useMetaOptions();
  const assign = useAssignConsignment(consignmentId);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <FieldShell label="Task" required error={errors.task_code}>
        {() => (
          <Combobox
            options={consignmentTaskCodeOptions}
            value={taskCode}
            onChange={(value) => {
              setTaskCode(value);
