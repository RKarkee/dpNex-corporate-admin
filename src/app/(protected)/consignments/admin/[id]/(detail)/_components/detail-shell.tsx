 ...
 * `useConsignment` and assume data — React Query serves it from cache, so that
 * second call costs nothing.
 *
 * The Edit button belongs here rather than in a tab: editing is a separate
 * page, not a section of this one. It stays commented out along with the rest
 * of the write path.
 */
export function DetailShell({
  ...
  const valid = Number.isFinite(consignmentId) && consignmentId > 0;

  const { data, isPending, isError, error, refetch } = useConsignment(
    valid ? consignmentId : 0,
  );
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            {/* {canUpdate ? (
              <Button asChild>
                <Link href={`/consignments/admin/${consignmentId}/edit`}>
