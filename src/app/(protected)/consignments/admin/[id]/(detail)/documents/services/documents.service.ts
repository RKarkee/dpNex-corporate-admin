...
/**
 * `/corporate/consignments/{id}/documents` — the paperwork attached to one
 * accepted consignment.
 *
 * A standalone sibling of the consignment *request* documents service, not a
 * reuse of it — same shape today, different resource, free to diverge.
...
function basePath(consignmentId: number | string): string {
  return `/corporate/consignments/${consignmentId}/documents`;
}

function docPath(consignmentId: number | string, docId: number | string): string {
  return `${basePath(consignmentId)}/${docId}`;
}
...
export async function fetchDocument(
  ...
  const raw = await privateApiClient.get<unknown>(docPath(consignmentId, docId), {
    silent: true,
    signal,
  });
...
export async function updateDocument(
  ...
  return privateApiClient.mutate("POST", docPath(consignmentId, docId), form, {
    silent: true,
  });
}
