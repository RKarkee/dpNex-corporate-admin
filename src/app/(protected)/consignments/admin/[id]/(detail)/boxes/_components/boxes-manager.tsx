...
/**
 * Boxes on the consignment detail page, each expanding to its items.
 *
 * Separate from the create/edit form on purpose. The form submits the whole
 * consignment in one body, which is right while it is being composed; once it
...
export function BoxesManager({
   ...
   *
   * `GET …/boxes/{boxId}` is what fills the dialog — not the row the user
   * clicked. The list endpoint returns a summary; the detail endpoint is the
   * only thing that promises the whole record, and reusing the row would show
   * whatever the list happened to include and silently omit the rest.
   */
  const [viewingBoxId, setViewingBoxId] = React.useState<number | null>(null);
  ...
  const viewed = viewBox.data;
  const viewRows: DetailRow[] = viewed
    ? [
        { label: "Box no.", value: viewed.box_no },
        { label: "Weight", value: show(viewed.weight, " kg") },
        { label: "Volumetric weight", value: viewed.volumetric_weight },
        { label: "Dimensions (L × W × H)", value: dimensions(viewed) },
        { label: "Pieces", value: viewed.no_of_pcs },
        {
          label: "Quantity code",
          value: optionLabel(quantityCodeOptions, viewed.quantity_code),
        },
        { label: "HS code", value: viewed.hs_code },
        {
          label: "Declared value",
          value: `${show(viewed.declared_value)} ${
            viewed.declared_currency ?? ""
          }`.trim(),
        },
        { label: "Items", value: viewed.items?.length ?? 0 },
        { label: "Goods description", value: viewed.goods_desc, full: true },
      ]
    : [];
  ...
  return (
    <Card>
            ...
            <TableBody>
              {isPending ? (
                ...
                <TableEmpty colSpan={COLUMNS}>
                  No boxes on this consignment yet.
                </TableEmpty>
              ) : (
                boxes.map((box, index) => {
                  // The API always sends an id; the index is a last resort so a
                  // malformed row still renders instead of collapsing keys.
                  ...
                  return (
                    <React.Fragment key={boxId}>
                      <TableRow className="bg-card">
                        ...
                        {/* Stored as a code (`PCS`); `/meta` carries the name. */}
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-0.5">
                            {/* Passes the id, not the row — the dialog fetches
                                `…/boxes/{boxId}` for itself. */}
                            <Button
                              variant="ghost"
                              size="icon-sm"
                              onClick={() => setViewingBoxId(boxId)}
      ...
      <DetailDialog
        ...
        onOpenChange={(open) => !open && setViewingBoxId(null)}
        title="Box details"
        // The request only starts when the dialog opens, so the spinner is the
        // honest first state rather than a flash of the previous box.
        loading={viewBox.isPending}
        rows={viewRows}
      />
