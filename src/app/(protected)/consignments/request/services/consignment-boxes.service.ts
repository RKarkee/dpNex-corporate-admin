 ...
 * without re-submitting the request around it.
 *
 * Two API quirks, both handled here rather than at the call sites:
 *
 ...
 *
 * These routes answer under the plural key holding one object — `data.boxes`
 * is the box, not a list of them — so an array is unwrapped to its first
 * entry rather than rejected.
 */
...
export async function fetchBox(
  ...
  const raw = await privateApiClient.get<unknown>(
    ENDPOINTS.box(consignmentId, boxId),
    { silent: true, signal },
  );
...
export async function listItems(
  ...
  const response = await privateApiClient.request<unknown>(
    "GET",
    ENDPOINTS.items(consignmentId, boxId),
    undefined,
...
export async function fetchItem(
  ...
  const raw = await privateApiClient.get<unknown>(
    ENDPOINTS.item(consignmentId, boxId, itemId),
    { silent: true, signal },
  );
