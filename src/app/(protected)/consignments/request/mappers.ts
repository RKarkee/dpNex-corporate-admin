 ...
 *
 * Results are cached per `(kind, code)` for the lifetime of one resolve pass,
 * so a currency repeated across thirty items costs one request, not thirty.
 */
 ...
 * the difference between "HS code 8517.12" and "Mobile phones" — never between
 * a usable form and an empty one. Blocking on a slow lookup endpoint would
 * leave someone staring at a skeleton for the full request timeout when the
 * record itself arrived long ago.
 */
 ...
 * requests go out together — a twenty-box request would otherwise resolve one
 * code at a time and take visibly long to open — and the whole batch is capped
 * by a deadline, after which whatever has resolved is kept and the rest stay as
 * codes.
 *
 * Late arrivals still land: each task writes into `data` as it settles, and
 * anything that finishes after the deadline simply updates an object the form
 * has not read yet, or has already read — harmless either way, since the form
 * copies these values on mount.
 *
 * Mutates and returns the object it was given; it is a fresh mapper output,
 * not shared state.
 */
