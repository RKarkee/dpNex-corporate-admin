 ...
 *
 * Generic over the form type because it serves three different forms with
 * three different field names: the rate check (`receiver_country`), and both
 * parties on the consignment form (`sender.country`, `receiver.country`).
 *
 ...
 *
 * Storing the state as a code creates two problems this component solves. The
 * city list needs that code, so a value that arrived as a *name* has to be
 * converted before cities can load — the first effect below. And the payload
 * wants the name alongside the code, which nothing else is tracking — the
 * second effect.
 */
...
export interface LocationFieldsProps<T extends FieldValues> {
  ...
  stateName: Path<T>;
  /**
   * The field that receives the readable state NAME. Defaults to
   * `${stateName}_name`, which is what the flat rate-check form uses; the
   * nested consignment form passes `sender.state_name` explicitly.
   */
...
export function LocationFields<T extends FieldValues>({
  ...
  const stateIso2 = iso2ToName[stateValue] ? stateValue : nameToIso2[stateValue] ?? "";
   ...
   * possible. `shouldDirty: false` keeps this invisible to the form's dirty
   * tracking — the user did not change anything, the value was already this
  ...
  React.useEffect(() => {
    ...
    // Already a known code — correct the casing and stop.
    ...
    // `nameToIso2` and `iso2ToName` are derived from the same fetch, so one is
    // enough to depend on; `setValue` and `stateName` are stable.
