 ...
 *
 * These are module-level constants rather than inline arrows because the
 * combobox lists `fetchPage` as a `useCallback` dependency — a new function
 * identity on every render would restart the debounce loop and re-request the
 * list continuously.
 */
