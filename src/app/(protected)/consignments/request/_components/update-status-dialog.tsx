...
import {
  ...
  requiresForwarder,
  requiresState,
} from "../[id]/(detail)/locations/types";
import { useUpdateRequestStatus } from "../_hooks/use-consignment-actions";
 ...
 * alike: this moves the request's own status, and only optionally records a
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Move this request to its next status, optionally recording where the
            shipment is.
          </DialogDescription>
        </DialogHeader>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        {statuses.length === 0 ? (
          <p className="-mt-2 text-xs text-muted-foreground sm:col-span-2">
            This request currently allows no status changes.
          </p>
        ) : null}
