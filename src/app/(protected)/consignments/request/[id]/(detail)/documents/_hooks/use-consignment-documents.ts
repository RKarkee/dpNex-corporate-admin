 ...
 *
 * Scoped by `requestId` so two consignment requests open in two tabs never
 * share a cache entry, and so invalidating one request's documents leaves the
 * other's alone.
 */
export const documentKeys = {
  all: (requestId: string) => ["consignment-documents", requestId] as const,
  list: (requestId: string, page: number, perPage: number) =>
    [...documentKeys.all(requestId), "list", { page, perPage }] as const,
  detail: (requestId: string, docId: number) =>
    [...documentKeys.all(requestId), "detail", docId] as const,
};
...
export function useConsignmentDocuments(
  ...
  return useQuery({
    queryKey: documentKeys.list(requestId, page, perPage),
    queryFn: ({ signal }) =>
      listDocuments({ requestId, page, perPage, signal }),
...
export function useConsignmentDocument(
  ...
  return useQuery({
    queryKey: documentKeys.detail(requestId, docId ?? -1),
    queryFn: ({ signal }) => fetchDocument(requestId, docId as number, signal),
...
export function useSaveConsignmentDocument(requestId: string) {
  ...
  return useMutation({
    ...
    mutationFn: async ({ id, input, selection, stored }: SaveDocumentVariables) =>
      id
        ? updateDocument(requestId, id, input, selection, stored ?? {})
        : createDocument(requestId, input, selection),
    ...
    onSuccess: async (result, variables) => {
      ...
      await queryClient.invalidateQueries({
        queryKey: documentKeys.all(requestId),
      });
    },
...
export function useDeleteConsignmentDocument(requestId: string) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (docId: number) => deleteDocument(requestId, docId),
    ...
    onError: (error) => handleDocumentError(error, "Could not delete document"),
  });
}
