...
export function DocumentsTab({ requestId }: { requestId: string }) {
  ...
  const [pendingDelete, setPendingDelete] =
    React.useState<ConsignmentDocument | null>(null);

  const query = useConsignmentDocuments(requestId, page, PER_PAGE);
  const saveDocument = useSaveConsignmentDocument(requestId);
  const deleteDocument = useDeleteConsignmentDocument(requestId);
    ...
    >
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          ...
          // user on a page that no longer exists, looking at an empty grid.
