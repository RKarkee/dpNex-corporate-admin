...
import { useCancelRequest } from "../../../../_hooks/use-consignment-actions";

/**
 * Cancel — `POST …/cancel` with `{ reason }`.
 *
 * The reason is recorded against the request and shown in its history, so it
...
export function CancelDialog({
  ...
  requestId: number;
  /** The tracking id, so the user sees which request they are cancelling. */
  label: string;
  open: boolean;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        {open ? (
          <CancelForm requestId={requestId} onDone={() => onOpenChange(false)} />
        ) : null}
      </DialogContent>
    </Dialog>
  );
...
function CancelForm({ requestId, onDone }: { requestId: number; onDone: () => void }) {
