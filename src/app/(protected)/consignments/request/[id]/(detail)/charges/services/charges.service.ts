...
/**
 * `/corporate/consignmentrequests/{id}/charges` — the cost line items.
 *
...
function basePath(requestId: number | string): string {
  return `/corporate/consignmentrequests/${requestId}/charges`;
}

function rowPath(requestId: number | string, chargeId: number | string) {
  return `${basePath(requestId)}/${chargeId}`;
}
...
export async function listCharges({
  ...
  const { items, meta } = await privateApiClient.paginated<unknown>(basePath(requestId), {
    params: {
      page,
...
export async function fetchCharge(
  ...
  const raw = await privateApiClient.get<unknown>(rowPath(requestId, chargeId), {
    silent: true,
    signal,
  });
...
export function createCharge(
  ...
  return privateApiClient.mutate("POST", basePath(requestId), toChargePayload(input), {
    silent: true,
  });
}
...
export function deleteCharge(
  ...
  return privateApiClient.mutate("DELETE", rowPath(requestId, chargeId), undefined, {
    silent: true,
  });
}
