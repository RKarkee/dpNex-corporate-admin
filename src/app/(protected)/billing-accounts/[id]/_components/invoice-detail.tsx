 ...
 *
 * `details` mixes two kinds of row under one array — `is_monetary: "N"` rows
 * are shipment facts (`MAWB/TKN/HAWB No.`, port of destination…) with no real
 * amount, `"Y"` rows are actual charges. Rendering them in one table would put
 * a `0.0000` rate next to a tracking number, so they're split into a facts
 * grid and a charges table instead — same underlying array, two views.
 */
            ...
            {invoice.consignment ? (
              <p className="flex flex-wrap items-center gap-1.5 pt-1 text-sm text-muted-foreground">
                ...
                <span className="font-medium text-foreground">
                  {invoice.consignment.tracking_no}
                </span>
                <span aria-hidden>·</span>
                {formatEnumLabel(invoice.consignment.status)}
              </p>
            ) : null}
          </div>
        ...
        <div className="overflow-hidden">
          <Table>
            ...
            <TableBody>
              ...
              {lines.map((line) => {
                // A `FIXED` line with no quantity is a flat charge — "0.000"
                // in that column would read as "we charged nothing per unit"
                // rather than "this charge has no unit".
